...
output. Handles both **inline** markup (`*…*`, `**…**`, `` `…` ``) and
**block** markup detected at the start of a line: ATX headers (`#`…`######`),
unordered/ordered lists, blockquotes, horizontal rules, and fenced code blocks
(``` ``` ``` / `~~~`) — the fence lines are consumed, the info string becomes a
language label, and the body is syntax-highlighted when Pygments is available
(dim otherwise).

The renderer is forward-only (no repaint). Block *markers* are the only thing
buffered, and only until the line's type is known — a plain prose line resolves
on its first character, so normal text still streams character-by-character.
Code-block bodies buffer one line at a time (needed to detect the closing fence
and to highlight). State survives chunk boundaries; `flush()` closes any open
span or unterminated fence so the terminal is never left styled.
"""
...
ITALIC = "\x1b[3m"
DIM = "\x1b[2m"
CODE = DIM  # inline code renders dim (name kept for back-compat with imports)
RESET = "\x1b[0m"
...
try:
    ...
    _FORMATTER: Any = Terminal256Formatter()
    _HIGHLIGHT = True
except Exception:  # pragma: no cover - pygments missing → graceful dim fallback
    _HIGHLIGHT = False

_LEXERS: dict[str, Any] = {}


def _highlight_code_line(line: str, lang: str) -> str | None:
    """Return `line` syntax-highlighted for `lang` (ANSI, no trailing newline),
    or None when highlighting is unavailable or the language is unknown."""
    if not _HIGHLIGHT or not lang:
        return None
    ...
    if lexer is None:
        return None
    return str(_pyg_highlight(line, lexer, _FORMATTER)).rstrip("\n")
...
class MarkdownStreamer:
    ...
    def _classify(self, s: str) -> Any:
        ...
        c = stripped[0]
        if c == "#":  # header candidate: `#`… awaiting the required space
            run = len(stripped) - len(stripped.lstrip("#"))
            return _MORE if stripped[run:] == "" and run <= 6 else ("text",)
        if c in "`~":  # 1-2 fence chars could still grow into a ``` / ~~~ fence
            run = len(stripped) - len(stripped.lstrip(c))
            return _MORE if run <= 2 and stripped[run:] == "" else ("text",)
        if c in "-+*_":  # bare marker → list / rule / emphasis still undecided
            if len(stripped) == 1:
                return _MORE  # could be "<marker> " list, or --- / *** / ___ rule
            if c in "-*_" and all(x == c for x in stripped):
                return _MORE  # possible horizontal rule
            return ("text",)
        if c.isdigit():  # ordered-list marker still forming (digits[.)] space)
    ...
    def _flush_text(self, s: str) -> None:
    ...
    def _is_close_fence(self, line: str) -> bool:
        s = line.strip()
        return bool(s) and all(c == self._fence_char for c in s) and len(s) >= self._fence_len

    def _render_code_line(self, line: str) -> None:
        painted = _highlight_code_line(line, self._fence_lang)
        self._out.write(painted if painted is not None else f"{CODE}{line}{RESET}")
    ...
    def _restyle(self) -> None:
        ...
        toggling individual attributes off (which terminals do inconsistently)."""
    ...
    def _end_line(self) -> None:
        """Close the current line: resolve a dangling `*`, reset open spans, and
        emit the newline. Inline spans do not cross lines (keeps the terminal
        from getting stuck styled on a mid-span line break)."""
