...
def live_app(tmp_path: Path, server: ModelServer, provider: config_mod.Provider, model: str) -> App:
    """The real app over `provider`, set to play `model`. The scripted
    ...
    return launch(root, server, spec=f"{provider.name}/{model}")
